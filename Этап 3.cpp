...
void computeUij(vector<vector<double>>& u,vector<vector<double>>& A,vector<vector<double>>& B,vector<vector<double>>& F,int& iterations,int rank,int size,
    ...
    while (norm_global > tolerance) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:local_R, local_AR)
        for (int i = 1; i <= local_M; ++i) {
            for (int j = 1; j <= local_N; ++j) {
                r[i][j] = -F[i][j]
...
int main(int argc, char* argv[]) {
    ...
    #pragma omp parallel for reduction(min:min_ratio_diff) 
    for (int i = 1; i <= int(sqrt(size)); i++) {
        if (size % i == 0) {
            ...
            if (ratio >= 1.0 && ratio <= 2.0) {
                double ratio_diff = abs(ratio - 1.0);
    ...
    if (rank == 0) {
        ...
        int idx = 0;
        #pragma omp parallel for private(idx) shared(u_global, u_total)
